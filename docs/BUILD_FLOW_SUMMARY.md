...
> [!summary]
> Grace Booth is implemented as an offline-first Windows Electron kiosk with a deterministic mock camera, encrypted local persistence, private Supabase delivery, and a Vercel-hosted public download page. The unsigned Windows installer and unpacked application were built and verified. No Supabase, Vercel, DNS, camera SDK, or other hosted resource was provisioned. Real Sony capture, live cloud validation, production signing, and testing on a supported production Windows host remain gated.
...
- `apps/public` — static React page intended for Vercel, using a URL fragment for the bearer token and POST-only photo APIs.
...
- Added a long-lived Sharp worker with signature/decode/orientation checks, 50 MiB and 80 MP limits, four-slot crop/fit/fill composition, frame-last rendering, sRGB output, 2700 px long edge, and JPEG quality 88.
...
- Added Frame Editor and Admin Settings, including validated transparent PNG frames, exactly four normalized slots, queue retry, passcode changes, optional Google Forms URL, health, and fixed retention display.
...
### Phase 3 — Supabase delivery and Vercel public page
...
- Added the Vercel public page. QR targets are `https://<public-origin>/photo#<token>`, so the fragment does not reach access logs. The page sends the token only in validated POST bodies and fetches controlled image/download bytes.
...
| ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Default Supabase Edge Function URLs rewrite HTML responses to plain text.                                   | A branded public HTML page could not be served from the locked default Function URL.                         | Approved a static Vercel public page; Supabase remains a POST-only private-media API.                                                                                                        |
...
| Early public build configuration required hosted environment values.                                        | A clean local `pnpm build` could fail before Supabase/Vercel existed.                                        | Added safe `.invalid` defaults for local builds. Real deployment must supply approved values and regenerate the exact CSP.                                                                   |
...
- No Vercel project, production origin, or DNS has been created or deployed.
- Hosted CORS/origin and end-to-end cloud smoke tests cannot run until both services exist.
- The checked-in public build intentionally points at `unconfigured.invalid`. Before deployment, set `VITE_PUBLIC_PHOTO_API_URL` and `VITE_PUBLIC_PAGE_ORIGIN`, configure Vercel with `apps/public` as the root, and regenerate/check the exact CSP.
...
4. Obtain approval to provision Supabase in Singapore and a Vercel project rooted at `apps/public`.
...
- `apps/public/README.md` — Vercel/public-page setup
